...
public class MainFrame extends JFrame {
	...
	public Double calculate1(Double x, Double y, Double z) {
		return Math.sin(Math.log(y)+Math.sin(y*y*Math.PI))*Math.pow(x*x +Math.sin(z)+Math.pow(Math.E, Math.cos(z)), 1/4);
	}
	public Double calculate2(Double x, Double y, Double z) {
		return (1+Math.pow(x, z)+Math.log(y*y))*(1-Math.sin(y*z))/(Math.pow(x*x*x+1, 1/2));
	}
